var Swiper = function(selector, params) {
        ...
        if (HTMLElement) {
            ...
            if (element.__defineGetter__) {
                element.__defineGetter__('outerHTML', function() {
                    return new XMLSerializer().serializeToString(this);//仅ff支持
                });
            }
        }
    }
    if (!window.getComputedStyle) {//getComputedStyle IE678不支持
    ...
    if (!Array.prototype.indexOf) {//indexOf IE678不支持
    ...
    if (!document.querySelectorAll) {//querySelectorAll IE67不支持
        if (!window.jQuery)//如果没有JQ选择器，返回出去
            return;
    }
    function $$(selector, context) {//选一组元素
        if (document.querySelectorAll)//支持querySelectorAll用querySelectorAll不支持用JQ选择器
...
Swiper.prototype = {plugins: {},wrapperTransitionEnd: function(callback, permanent) {
    ...
    },h: {getWidth: function(el, outer, round) {
            'use strict';
